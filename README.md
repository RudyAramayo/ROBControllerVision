...
10. For Cerebro/Gemini action proposals, explicitly enable **Cerebro Action Approval**. Inspect each immutable, expiring proposal and choose **Approve** or **Reject**. Gesture approval authorizes one named catalog gesture; it never accepts model-supplied joint values. A `run_startup_test` proposal is Cerebro's separately armed monitorless workflow: Approve is the final safety checkpoint and starts one fixed measured two-arm test after an audible warning, with no typing or droid display.
