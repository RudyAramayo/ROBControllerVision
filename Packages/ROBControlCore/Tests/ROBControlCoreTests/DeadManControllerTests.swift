...
struct DeadManControllerTests {
    ...
    func exactTimeoutBoundary() {
        ...
        #expect(
            ...
            ) == .drive(MotionVector(linear: 0.7, angular: -0.2), .centered, .init(isActive: true), .inactive, nil)
        )
        #expect(
            controller.evaluate(
                connectionIsReady: true,
    ...
    func freshInputClearsInvalidation() {
        ...
        #expect(
            controller.evaluate(connectionIsReady: true, at: origin)
                == .drive(MotionVector(linear: 0.5, angular: 0), .centered, .init(isActive: true), .inactive, nil)
        )
    }
}
